...
class HouseCanaryClient(object):
    ...
    def get(self, api, address, zipcode, output_format="all", report_type="full"):
        ...
        Args:
            ...
            report_type (optional) -- Type of report.
                Can be 'summary' or 'full'. The default is 'full'.
        ...
        if api == "value_report":
            query_params["format"] = output_format or "all"
    ...
    def get_multi(self, api, address_list, output_format="all", report_type="full"):
        ...
        Args:
            ...
            output_format (optional) -- Output format. Can be 'json', 'pdf' or 'all'.
                The default is 'all', which is a zip of all available formats.
                output_format is only used when api is set to 'value_report'
            report_type (optional) -- Type of report. Can be 'summary' or 'full'.
                The default is 'full'.
                report_type is only used when api is set to 'value_report'
        ...
        data_structure = self._get_post_data_structure(address_list)
    ...
    def _check_api_param(self, api):
        if api.lower() not in self.AVAILABLE_APIS:
            raise ValueError("api param is invalid. It must be one of " +
                             ", ".join(self.AVAILABLE_APIS))
        return True

    @staticmethod
    def _get_post_data_structure(address_list):
        if not address_list:
        ...
        for address in address_list:
            if isinstance(address, HouseCanaryProperty):
                hc_address = address
            elif isinstance(address, list):
                hc_address = HouseCanaryProperty(address[0], address[1])
            else:
                msg = ("Items in address_list must be lists of strings like "
    ...
    def _make_request(self, endpoint, query_params, method, data):
        ...
        try:
            ...
            error_json = json.loads(ex.read())

            if ex.getcode() == HTTP_STATUS_FORBIDDEN:
                ex.msg = "You are not authenticated. " + ex.msg
                ...
                ex.msg = ". ".join(error_messages) + ". " + ex.msg
            raise
